...
def myupdate(mymodel, values):
    ...
    # values(**values) :values、つまり全てのカラムに記載されている情報を選択する
    query = update(mymodel).where(mymodel.customer_id == customer_id).values(**values) # "お見事！E0002の原因はこのクエリの実装ミスです。正しく実装しましょう"
    try:
        # トランザクションを開始
        with session.begin():
            result = session.execute(query)
